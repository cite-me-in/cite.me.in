...
describe("fetchOrganicResults (google)", () => {
  it("should return organic URLs from the first page", async () => {
    ...
    const result = await fetchOrganicResults("best retail space platforms", "google");
  ...
  it("should return empty array when no organic results", async () => {
    ...
    const result = await fetchOrganicResults("very niche query", "google");

    expect(result).toEqual([]);
  });

  it("should use engine=google in the request", async () => {
    ...
    await fetchOrganicResults("test query", "google");
  ...
  it("should throw when the API response is not ok", async () => {
    ...
    await expect(
      fetchOrganicResults("query", "google"),
