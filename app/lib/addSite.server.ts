...
import { setStatus } from "~/lib/setupProgress.server";

/**
 * Add a site to the database and run the setup pipeline.
 *
 * @param site - The site to add.
 * @param user - The user who is adding the site.
...
export default async function addSite({
  ...
  try {
    ...
    // Phase 6: AI Legibility scan (runs in background)
    await runAILegibilityScan({ log, site, user });
