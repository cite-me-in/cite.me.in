...
**Goal:** Add outbound webhooks that fire on key events (`user.created`,
`site.created`, `site.deleted`), with DB-backed delivery, HMAC signing, and
3-attempt retry via cron.

**Architecture:** Two new Prisma models (`WebhookEndpoint`, `WebhookDelivery`)
store registered recipients and delivery records. A single
`emitWebhookEvent(type, payload)` helper handles all emission logic, routing
events to the right endpoints based on event scope (admin-only vs. site-owner +
admin). A cron route processes RETRY deliveries every 5 minutes.

**Tech Stack:** Prisma (PostgreSQL), Node.js `crypto` module for HMAC, `fetch`
with `AbortSignal.timeout` for delivery, MSW for test HTTP mocking.
...
Add to `prisma/schema.prisma` — place new models alphabetically and the enum
near the bottom with other enums:
...
Add `webhookEndpoints WebhookEndpoint[]` to the `User` model (after the last
existing relation field).
...
Expected: no errors, types for `WebhookEndpoint`, `WebhookDelivery`,
`DeliveryStatus` are available.
...
The cron test hits the running server with `fetch`. Follow the same pattern as
`test/routes/cron.process-sites.test.ts`.
...
The route uses `loader` (GET) to match the existing cron convention in this
project.
...
Expected: all PASS. The RETRY deliveries pointing to `https://admin.test/hook`
will get a 503 from MSW's catch-all and be rescheduled — the test only checks
`processed` count, not final status.
...
In `test/routes/sign-up.test.ts`, find the test that verifies successful sign-up
(the one that fills the form and expects a redirect or the `/sites` page). Add a
`beforeEach` that seeds an admin webhook endpoint, and add a DB assertion at the
end of the success test.
...
In `app/routes/sign-up.tsx`, add after the `prisma.user.create` call (before the
redirect):
...
`emitWebhookEvent` has its own internal try/catch — sign-up will not break if
...
```ts
import { vi, expect, it, describe, beforeEach, afterEach } from "vite-plus/test";
...
In `deleteSite`, emit BEFORE `prisma.site.delete` (the site must exist for
`emitWebhookEvent` to look up the owner):
...
- Create: `scripts/seed-admin-webhook.ts` (not committed — run once against
  prod)
...
```bash
infisical --env prod run -- psql $DATABASE_URL_NON_POOLING -c "SELECT id, email FROM users WHERE is_admin = true;"
```
...
```bash
infisical --env prod run -- psql $DATABASE_URL_NON_POOLING -c "SELECT id, url, events FROM webhook_endpoints;"
```
